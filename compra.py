...
class Operaciones:
    ...
    def insertar(self, ComIdeFisEst, ComMar, ComFil, ComCol, ComClaTra, ComCarMen, ComFec, ComCan, ComPre):
        cur = self.cnn.cursor()
        sql = "INSERT INTO {} (ComIdeFisEst, ComMar, ComFil, ComCol, ComClaTra, ComCarMen, ComFec, ComCan, ComPre) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)".format(NOMBRE_TABLA)
        cur.execute(sql, (ComIdeFisEst, ComMar, ComFil, ComCol, ComClaTra, ComCarMen, ComFec, ComCan, ComPre))
        n = cur.rowcount
        self.cnn.commit()
        cur.close()
        return n
    
    def eliminar(self, ComIdeFisEst, ComMar, ComFil, ComCol, ComClaTra, ComCarMen, ComFec):
        cur = self.cnn.cursor()
        sql = "DELETE FROM {} WHERE ComIdeFisEst = %s AND ComMar = %s AND ComFil = %s AND ComCol = %s AND ComClaTra = %s AND ComCarMen = %s AND ComFec = %s".format(NOMBRE_TABLA)
        cur.execute(sql, (ComIdeFisEst, ComMar, ComFil, ComCol, ComClaTra, ComCarMen, ComFec))
        n = cur.rowcount
        self.cnn.commit()
        cur.close()
        return n

    def modificar(self, ComIdeFisEst, ComMar, ComFil, ComCol, ComClaTra, ComCarMen, ComFec, ComCan, ComPre):
        cur = self.cnn.cursor()
        sql = "UPDATE {} SET ComCan = %s, ComPre = %s WHERE ComIdeFisEst = %s AND ComMar = %s AND ComFil = %s AND ComCol = %s AND ComClaTra = %s AND ComCarMen = %s AND ComFec = %s".format(NOMBRE_TABLA)
        cur.execute(sql, (ComCan, ComPre, ComIdeFisEst, ComMar, ComFil, ComCol, ComClaTra, ComCarMen, ComFec))
        n = cur.rowcount
        self.cnn.commit()
        cur.close()
        return n
...
class Ventana(Frame):
    ...
    def create_widgets(self):
        ...
        nombre_columnas = ["Id Fiscal", "Marca", "Filtro", "Color", "Clase de Tratamiento", "Mentolado", "Fecha Compra", "Cantidad", "Precio Compra"]
