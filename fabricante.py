...
class Operaciones:
    ...
    def insertar(self, nombre, pais):
        cur = self.cnn.cursor()
        sql = "INSERT INTO {} (FabNom, FabPai) VALUES (%s, %s)".format(NOMBRE_TABLA)
        cur.execute(sql, (nombre, pais))
        n = cur.rowcount
        self.cnn.commit()
        cur.close()
        return n
    ...
    def eliminar(self, nombre):
        cur = self.cnn.cursor()
        sql = "DELETE FROM {} WHERE FabNom = %s".format(NOMBRE_TABLA)
    ...
    def modificar(self, nombre, pais):
        cur = self.cnn.cursor()
        sql = "UPDATE {} SET FabPai=%s WHERE FabNom=%s".format(NOMBRE_TABLA)
        cur.execute(sql, (pais, nombre))
        n = cur.rowcount
        self.cnn.commit()
        cur.close()
        return n
...
class Ventana(Frame):
    ...
    def llenaDatos(self):
        ...
        for row in datos:            
            self.grid.insert("",END,text=row[0], values=(row[0], row[1]))
    ...
    def fGuardar(self): 
        if self.id ==-1:       
            self.operacion.insertar(self.txtNombre.get(),self.txtPais.get())            
            messagebox.showinfo("Insertar", 'Elemento insertado correctamente.')
        else:
            self.operacion.modificar(self.txtNombre.get(),self.txtPais.get())
    ...
    def create_widgets(self):
        ...
        lbl2 = Label(frame2,text="Pais: ")
        lbl2.place(x=3,y=self.aumentarPosEtiqueta())        
        self.txtPais=Entry(frame2)
        self.txtPais.place(x=3,y=self.aumentarPosCampo(),width=130, height=20)        
        ######################################################################################################
